...
class Product 
	...
	attr_accessor :title, :quantity, :serial_number, :cost, :sell_price

	def initialize(title, quantity, serial_number, cost, sell_price)
		@title = title
		@quantity = quantity
...
#For Cateogories
class Category 
	# one of the attributes of Category is an array with objects from another custom class: Product
	attr_accessor :title, :products
...
#(these methods are to 'seed' the database so it has info to work with)
